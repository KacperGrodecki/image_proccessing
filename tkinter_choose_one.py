...
class Window(tk.Frame):
    ...
    def __init__(self,master,imageProcessing):
        ...
        self.xywh=[]
        self.xywh1=[]
        self.path='/home/kacper/Dokumenty/GitHub/image_proccessing/res500/'  
    ...
    def open_file(self):
        self.file,self.image=self.opening(self.file_nr)
    ...
    def next_contour(self):
        ...
        if self.countter>len(self.contours)-2:
            print('finish')
            print('closing')
            self.labels.append([self.file_nr,'','',''])
            self.model()
            #self.file_nr+=1
            #self.countter=0
            #self.countter1=0
            print(self.labels)
            print(self.file_nr,' ',self.files[self.file_nr])
            self.client_exit()
        self.countter+=1
        self.read_text()
    ...
    def analyze_figures(self,xywh,cropped_board,mask,mask1):
        ...
        let_mask=image_to_string(mask, lang='pol',config='--psm 7 --oem 3')#dodac inne psm
...
app = Window(root,imageProcessing)
root.mainloop()  
